...
Prefer `seller_campaign_context` first. Use the tool output as planning context, not as the final answer. If the tool says more input is needed or the product match is ambiguous, ask the user to fill the gap before planning. If useful, call `seller_inventory_lookup` or `seller_restock_signal` for extra context.
